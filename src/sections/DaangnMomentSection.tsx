...
export function DaangnMomentSection() {
  return (
    <section
      ...
      className='mo:py-[11.875rem] ta:px-[2.5rem] min-h-[100dvh] bg-white px-[1rem] py-[5rem]'
    >
      <div className='sde:max-w-[60.625rem] mx-auto flex max-w-[35.125rem] flex-col items-center'>
        <h2
          // className='whitespace-pre-line text-center font-karrot text-[3.75rem] leading-tight'
          className='mo:text-[3.125rem] ta:text-[3.75rem] whitespace-pre-line text-center font-karrot text-[1.75rem] leading-tight antialiased'
        >
          동네에서 만나는{'\n'}당근의 순간들
        </h2>
        <div
          // className='mt-6 text-center font-karrot text-[1.375rem] text-[#4d5159]'
          className='mo:mt-[1.5rem] mo:text-[1.25rem] ta:text-[1.375rem] mt-[1.25rem] text-center font-karrot text-[1rem] text-[#4d5159] antialiased'
        >
          <p>당근하는 순간이 많아질수록</p>
        ...
        <div
          // className='mt-[5.625rem] grid max-w-[60.625rem] grid-flow-row-dense grid-cols-2 grid-rows-[1fr_0.3fr_0.7fr_1fr] content-start gap-[1rem]'
          className='mo:mt-[5.625rem] sde:max-w-[50rem] de:max-w-full sde:grid sde:grid-flow-row-dense sde:grid-cols-2 sde:grid-rows-[1fr_0.3fr_0.7fr_1fr] sde:content-start mt-[3.125rem] flex flex-col items-start gap-[1rem]'
        >
          <article className='relative order-1 transition ease-out hover:scale-[1.01]'>
            <h2
              // className='absolute left-0 top-[2.5rem] z-10 w-full text-center font-karrot text-[1.75rem]'
              className='mo:text-[1.5rem] ta:text-[1.75rem] ta:top-[2.5rem] ta:leading-normal sde:text-[1.5rem] sde:order-1 absolute left-0 top-[1.875rem] z-10 w-full text-center font-karrot text-[1.25rem] antialiased'
            >
              좋은 물건을 구하고 <br />
              나누는 중고거래
            </h2>
